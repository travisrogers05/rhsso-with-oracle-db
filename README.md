...
This example adds an Oracle JDBC driver as a module into a Red Hat SSO image during an Openshift based source to image (s2i) build.  A datasource is created at deploy time that uses the Oracle JDBC driver.  There are a few mechanisms used that are provided as facilities in the RH SSO image for Openshift from Red Hat.
